...
def compute_multiple( nmrObj, data_parent_folder, meas_folder, file_name_prefix, en_fig, en_ext_rotation, thetaref, en_ext_matchfilter, echoref_avg, dconv_lpf_ord, dconv_lpf_cutoff_kHz, ignore_echoes ):
    ...
    en_rotation = 1 # perform rotation to the data -> required for reference data for t1 measurement
    ...
    unit__uvolt_or_digit = 0 # set to 0 of ADC digit unit, or 1 for microvolt unit for all the plot 

    compute_figure = True  # compute the figure and save the figure to file. To show it in runtime, enable the en_fig
    save_indv_dat = False # save individual computed data
    ...
    def exp_func( x, a, b ): # single exponential
        return a * np.exp( -b * x )
    ...
    try:  # try fitting data
        popt, pocv = curve_fit( exp_func, t_echospace[ignore_echoes:], np.real(a[ignore_echoes:]), guess )
